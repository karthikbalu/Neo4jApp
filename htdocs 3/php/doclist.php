...
/*
$sqlinterest="SELECT r.int_group_id,r.usrid,
 (SELECT r1.yt_interest FROM yt_usr_interest r1 WHERE r.int_group_id ORDER BY rand() LIMIT 1) AS 'yt_interest' FROM yt_usr_interest r
WHERE usrid=".$usrid." AND r.int_group_id = '0' GROUP BY r.int_group_id LIMIT 7";
*/

$sqlinterest="select * from yt_usr_interest where usrid='".$usrid."' order by yt_tmm DESC LIMIT 7";
...
echo $sqlinterest;
	...
	///$info23 = mysql_fetch_array($data231);

	while ($info231 = mysql_fetch_array($data231)) {
		$interest_slist[] = $info231['yt_interest'];
		$cur_interest = $info231['yt_interest'];
		//$cur_interest_id = $info231['int_group_id'];
        }
...
//$op="ajith";
 ...
 $base = "http://clofus.com:7771/dcs/rest?dcs.source=etools&query=".$op."&results=".$aa[$j]."&dcs.algorithm=lingo&dcs.output.format=JSON";
echo $base;
